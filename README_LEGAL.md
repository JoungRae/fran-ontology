...
3. **실 유형 판정** — 실명이 물리적으로 '실'인지 '샤프트'인지 별도로 묻는다.
   규칙 해석과 분리해야 안정적이다.
4. **컴파일** — deontic(obligation/permission)·requirement 에 더해 **면제
   연결**(cons_law `rule_override`, 2단계 산출물)로 동작을 정한다:
   - permission 중 **exempt 연결이 있는 것만** 제외 후보. 연결 없는 permission
     은 택일("합성수지배관으로 설치할 수 있다")이라 아무 의무도 안 끈다 —
     전에는 이 구분이 없어 택일 규칙까지 생략가능 근거로 실렸다.
   - 반경 후보 중 이 실에 함께 걸린 규칙이 replace/relax 로 덮은 의무는 탈락.
   - 각 생략가능 근거에 `끄는의무`(꺼지는 의무 규칙 id)를 남긴다 — "왜 이 실은
     제외 가능한가"가 규칙 그래프에서 끝까지 추적된다.
